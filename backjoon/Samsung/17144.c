...
void spreadDust(int i, int j){
   ...
   afterDust=beforeDust-(spreadDustValue * spreadCnt);
   afterDust = afterDust>0?afterDust:0;
//    printf("#######\n beforeDust[%d], spreadDustValue[%d], spreadCnt[%d], afterDust[%d] spreadDustValue*spreadCnt[%d]\n",beforeDust,spreadDustValue,spreadCnt, afterDust,spreadDustValue*spreadCnt);

for (size_t k = 0; k < 4; k++)
{
    if(availableDirection[k]){
      sequenceMap[(i+directionRow[k])][(j+directionCol[k])]+=spreadDustValue;
    // printf(" [%d]:%d \n",k,sequenceMap[(i+directionRow[k])][(j+directionCol[k])]);
...
int sol(int time){
    for (size_t i = 0; i < time; i++)
    {
        ...
        nowMap[locCirculator[0]][locCirculator[1]]=-1;
        nowMap[locCirculator[2]][locCirculator[3]]=-1;
        // printf("After dustFlowAction\n");
        // printMap();
        blowWind();
        // printf("After blowWind\n");
